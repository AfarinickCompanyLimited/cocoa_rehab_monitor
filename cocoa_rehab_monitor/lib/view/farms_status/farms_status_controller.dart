...
class FarmsStatusController extends GetxController{

  BuildContext? farmsStatusScreenContext;
  ...
  final PagingController<int, FarmStatus> pagingController = PagingController(firstPageKey: 0);
  ...
  Future<void> fetchData({required int status, required int pageKey, required PagingController controller}) async {
    try {
      final data = searchTC!.text.isEmpty ?
                  await globalController.database!.farmStatusDao.findFarmStatusWithLimit(_pageSize, pageKey * _pageSize)
                : await globalController.database!.farmStatusDao.findFarmStatusWithSearchAndLimit("%${searchTC!.text.trim()}%", _pageSize, pageKey * _pageSize);
    ...
    FirebaseCrashlytics.instance.recordError(error, stackTrace);
        FirebaseCrashlytics.instance.log('farms status. fetchData');
