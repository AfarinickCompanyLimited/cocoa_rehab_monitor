...
class _PersonnelHistoryState extends State<PersonnelHistory> with SingleTickerProviderStateMixin {
  ...
  @override
  void initState() {
    personnelHistoryController.pendingRecordsController.addPageRequestListener((pageKey) {
      personnelHistoryController.fetchPersonnel(status: SubmissionStatus.pending, pageKey: pageKey, controller: personnelHistoryController.pendingRecordsController);
    });

    personnelHistoryController.submittedRecordsController.addPageRequestListener((pageKey) {
      personnelHistoryController.fetchPersonnel(status: SubmissionStatus.submitted, pageKey: pageKey, controller: personnelHistoryController.submittedRecordsController);
    });
                    ...
                    children: [
                      ...
                      GetBuilder(
                        ...
                        builder: (ctx) {
                          return PagedListView<int, Personnel>(
                            ...
                            pagingController: personnelHistoryController.submittedRecordsController,
                            builderDelegate: PagedChildBuilderDelegate<Personnel>(
                      ...
                      GetBuilder(
                        ...
                        builder: (ctx) {
                          return PagedListView<int, Personnel>(
                            padding: const EdgeInsets.only(left: 15, right: 15, bottom: 20, top: 15),
                            pagingController: personnelHistoryController.pendingRecordsController,
                            builderDelegate: PagedChildBuilderDelegate<Personnel>(
                                itemBuilder: (context, personnel, index){
                                  return PersonnelCard(
                                    personnel: personnel,
                                    ...
                                    onEditTap: () {
                                      Get.to(() =>
                                        ...
                                        if (data != null){
                                          var updatedPersonnel = data['personnel'];
                                          bool submitted = data['submitted'];
                                          // personnelHistoryController.pendingRecordsController.updateItem(index, updatedPersonnel);

                                          final index = personnelHistoryController.pendingRecordsController.itemList?.indexWhere((p) => p.uid == updatedPersonnel.uid);
                                          if (index != -1) {
                                            if(submitted){
                                              personnelHistoryController.pendingRecordsController.itemList!.remove(personnel);
                                              personnelHistoryController.update(['pendingRecordsBuilder']);
                                              personnelHistoryController.submittedRecordsController.refresh();
                                            }else{
                                              personnelHistoryController.pendingRecordsController.itemList![index!] = updatedPersonnel;
                                              personnelHistoryController.update(['pendingRecordsBuilder']);
                                            }
                                          }
                                        }
