...
import 'dart:convert';
import 'dart:io';
import 'package:cocoa_rehab_monitor/controller/api_interface/parse_image.dart';
import 'package:cocoa_rehab_monitor/controller/model/cm_user.dart';
import 'package:cocoa_rehab_monitor/controller/model/user_info.dart';
...
import 'package:cocoa_rehab_monitor/view/auth/login/login.dart';
...
class UserInfoApiInterface {
    ...
    if (await ConnectionVerify.connectionIsAvailable()) {
      try {
        var response = await dio.post(
          URLs.baseUrl + URLs.lookupUserAccount,
  ...
  createAccount(userData) async {
    ...
    if (await ConnectionVerify.connectionIsAvailable()) {
      try {
        var response = await dio.post(
          URLs.baseUrl + URLs.createAccount,
          data: userData,
        );
  ...
  updateProfilePhoto(data, token) async {
    ...
    if (await ConnectionVerify.connectionIsAvailable()) {
      try {
        var response = await dio.post(
          URLs.baseUrl + URLs.updateProfilePhoto,
          data: data,
        );
        if (response.data['status'] == true) {
  ...
  updateAccountDetails(userData, token) async {
    ...
    if (await ConnectionVerify.connectionIsAvailable()) {
      try {
        var response = await dio.post(
          URLs.baseUrl + URLs.updateAccountDetails,
          data: userData,
        );
        var data = response.data;
  ...
  //         URLs.baseUrl + URLs.updateFirebaseToken,
  //         data: {
  //           'token': token,
