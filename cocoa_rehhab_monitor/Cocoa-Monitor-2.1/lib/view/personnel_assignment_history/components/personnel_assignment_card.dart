...
class PersonnelAssignmentCard extends StatelessWidget {
  ...
  const PersonnelAssignmentCard({Key? key, required this.personnelAssignment, this.onViewTap, this.onEditTap, this.onDeleteTap, required this.allowEdit, required this.allowDelete}) : super(key: key);
      ...
      child: Column(
        ...
        children: [
          FutureBuilder(
            builder: (ctx, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                ...
                if (snapshot.hasError) {
                  ...
                  List<Farm> dataList = snapshot.data as List<Farm>;
                  if (dataList.isNotEmpty) {
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Text('${dataList.first.farmerNam}',
                          style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: AppColor.black),
                        ),
                        const SizedBox(height: 2),
                        Text('Ref: ${dataList.first.farmId}',
            ...
            future: globalController.database!.farmDao.findFarmByFarmCode(int.parse(personnelAssignment.farmid!)),
          ),
          ...
          FutureBuilder(
            ...
            future: globalController.database!.activityDao.findActivityByCode(personnelAssignment.activity!),
          ),
          const SizedBox(height: 10),
          Text(personnelAssignment.assignedDate!,
