...
class OutbreakFarmHistoryController extends GetxController{

  BuildContext? outbreakFarmHistoryScreenContext;
  ...
  final PagingController<int, OutbreakFarm> pendingRecordsController = PagingController(firstPageKey: 0);
  final PagingController<int, OutbreakFarm> submittedRecordsController = PagingController(firstPageKey: 0);
  ...
  Future<void> fetchData({required int status, required int pageKey, required PagingController controller}) async {
    try {
      final data = await globalController.database!.outbreakFarmDao.findOutbreakFarmByStatusWithLimit(
  ...
  confirmDeleteRecord(OutbreakFarm outbreakFarm) async {
    globals.primaryConfirmDialog(
        context: outbreakFarmHistoryScreenContext,
        title: 'Delete Entry',
        ...
        okayTap: () {
          Get.back();
          globalController.database!.outbreakFarmDao.deleteOutbreakFarmByUID(outbreakFarm.uid!);

          pendingRecordsController.itemList!.remove(outbreakFarm);
