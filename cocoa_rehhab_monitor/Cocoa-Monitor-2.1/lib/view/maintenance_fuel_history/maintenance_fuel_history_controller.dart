...
class MaintenanceFuelHistoryController extends GetxController{
  ...
  final PagingController<int, MaintenanceFuel> pendingRecordsController = PagingController(firstPageKey: 0);
  final PagingController<int, MaintenanceFuel> submittedRecordsController = PagingController(firstPageKey: 0);
  ...
  Future<void> fetchData({required int status, required int pageKey, required PagingController controller}) async {
    try {
      final data = await globalController.database!.initialTreatmentMonitorDao.findInitialTreatmentMonitorByStatusWithLimit(
  ...
  confirmDelete(MaintenanceFuel maintenanceFuel) async {
    globals.primaryConfirmDialog(
        ...
        okayTap: () {
          Get.back();
          globalController.database!.maintenanceFuelDao.deleteMaintenanceFuelByUID(maintenanceFuel.uid!);
          // update();
          pendingRecordsController.itemList!.remove(maintenanceFuel);
